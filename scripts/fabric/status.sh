...
# --- EVPN overlay bridges (informational; not scored fatally) ---
echo ""
echo "Overlay Bridges (5):"
OVL_UP=0
OVL_TOTAL=5
if ip link show br-ovl-01 >/dev/null 2>&1; then
    OVL_UP=$((OVL_UP + 1))
fi
if ip link show br-ovl-05 >/dev/null 2>&1; then
    OVL_UP=$((OVL_UP + 1))
fi
if ip link show br-ovl-09 >/dev/null 2>&1; then
    OVL_UP=$((OVL_UP + 1))
fi
if ip link show br-ovl-0D >/dev/null 2>&1; then
    OVL_UP=$((OVL_UP + 1))
fi
if ip link show br-ovl-0D-b >/dev/null 2>&1; then
    OVL_UP=$((OVL_UP + 1))
fi
if [ "$OVL_UP" -eq "$OVL_TOTAL" ]; then
    ok "All $OVL_TOTAL overlay bridges up"
else
    warn "$OVL_UP / $OVL_TOTAL overlay bridges up"
fi
...
echo ""
# --- EVPN overlay health (from leaf-1a; informational, not scored) ---
echo "EVPN Overlay (from leaf-1a):"
STATE=$(virsh -c qemu:///system domstate "${VIRSH_PREFIX}_leaf-1a" 2>/dev/null || echo "not found")
if [ "$STATE" = "running" ]; then
    cd "$PROJECT_ROOT"
    VNI_OUT=$(vagrant ssh leaf-1a -c "sudo vtysh -c 'show evpn vni'" 2>/dev/null || true)
    if echo "$VNI_OUT" | grep -qE '\<10000\>'; then
        ok "leaf-1a: L2VNI 10000 present"
    else
        warn "leaf-1a: L2VNI 10000 not yet in 'show evpn vni' (BGP-EVPN may still be converging)"
    fi
    if echo "$VNI_OUT" | grep -qE '\<10999\>'; then
        ok "leaf-1a: L3VNI 10999 present"
    else
        warn "leaf-1a: L3VNI 10999 not yet present (symmetric IRB still converging?)"
    fi
    REMOTE=$(vagrant ssh leaf-1a -c "sudo vtysh -c 'show evpn mac vni 10000'" 2>/dev/null | grep -ic remote || true)
    if [ "${REMOTE:-0}" -gt 0 ]; then
        ok "leaf-1a: $REMOTE remote MAC(s) learned on VNI 10000"
    else
        warn "leaf-1a: no remote MACs on VNI 10000 yet (run 'make overlay'; allow convergence)"
    fi
else
    warn "leaf-1a not running -- skipping EVPN overlay check"
fi
