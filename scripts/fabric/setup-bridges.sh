...
echo "  [overlay] br-ovl-01 (tenant access segment)"
sudo ip link add br-ovl-01 type bridge 2>/dev/null || true
sudo bash -c 'echo 0 > /sys/class/net/br-ovl-01/bridge/stp_state'
sudo ip link set br-ovl-01 up
echo "  [overlay] br-ovl-05 (tenant access segment)"
sudo ip link add br-ovl-05 type bridge 2>/dev/null || true
sudo bash -c 'echo 0 > /sys/class/net/br-ovl-05/bridge/stp_state'
sudo ip link set br-ovl-05 up
echo "  [overlay] br-ovl-09 (tenant access segment)"
sudo ip link add br-ovl-09 type bridge 2>/dev/null || true
sudo bash -c 'echo 0 > /sys/class/net/br-ovl-09/bridge/stp_state'
sudo ip link set br-ovl-09 up
echo "  [overlay] br-ovl-0D (tenant access segment)"
sudo ip link add br-ovl-0D type bridge 2>/dev/null || true
sudo bash -c 'echo 0 > /sys/class/net/br-ovl-0D/bridge/stp_state'
sudo ip link set br-ovl-0D up
echo "  [overlay] br-ovl-0D-b (tenant access segment)"
sudo ip link add br-ovl-0D-b type bridge 2>/dev/null || true
sudo bash -c 'echo 0 > /sys/class/net/br-ovl-0D-b/bridge/stp_state'
sudo ip link set br-ovl-0D-b up

echo ""
echo "NetWatch: 54 fabric bridges + 5 overlay bridges created (STP disabled)."
