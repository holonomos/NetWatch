...
OUTPUT_BOX="$PROJECT_ROOT/artifacts/boxes/netwatch-golden.box"
...
source "$PROJECT_ROOT/artifacts/versions.env"

# Box filename and registered name derive from FEDORA_RELEASE so they always
# agree with versions.env and with what scripts/artifacts/build.sh produced.
FEDORA_BOX_NAME="fedora-${FEDORA_RELEASE}.box"
FEDORA_BOX="$PROJECT_ROOT/artifacts/boxes/${FEDORA_BOX_NAME}"
BASE_BOX_NAME="netwatch-fedora${FEDORA_RELEASE}"

# FRR is installed from the Fedora repos (see Phase 1b). FRR_VERSION pins the
# package for reproducible bakes; empty/"latest" falls back to plain `dnf install frr`.
FRR_VERSION="${FRR_VERSION:-}"

echo "=== NetWatch Golden Image Builder ==="
echo ""
echo "  Base box:     ${BASE_BOX_NAME} (artifacts/boxes/${FEDORA_BOX_NAME})"
echo "  FRR:          ${FRR_VERSION:-latest} (Fedora repos)"
...
if ! vagrant box list | grep -q "$BASE_BOX_NAME"; then
  echo "Base box not registered. Adding artifacts/boxes/${FEDORA_BOX_NAME}..."
  vagrant box add --name "$BASE_BOX_NAME" "$FEDORA_BOX"
fi
...
vagrant ssh -c "sudo env FRR_VERSION='${FRR_VERSION}' bash -s" <<'PROVISION_FRR'
set -euo pipefail

# FRR_VERSION pins the package for reproducible bakes. If the exact version is
# no longer carried by the Fedora repo, fall back to the repo's current FRR so
# the bake never hard-fails on a stale pin.
if [ -n "${FRR_VERSION:-}" ]; then
  echo "--- Installing FRR ${FRR_VERSION} from Fedora repos (pinned) ---"
  if ! dnf install -y "frr-${FRR_VERSION}"; then
    echo "WARNING: frr-${FRR_VERSION} unavailable in Fedora repos; falling back to latest frr"
    dnf install -y frr
  fi
else
  echo "--- Installing FRR (latest) from Fedora repos ---"
  dnf install -y frr
fi
...
# ==========================================================================
# Phase 2: Binary artifacts (observability)
# ==========================================================================
echo ""
echo "=== Phase 2: Binary artifacts ==="

vagrant ssh -c "sudo env NODE_EXPORTER_VERSION='${NODE_EXPORTER_VERSION}' FRR_EXPORTER_VERSION='${FRR_EXPORTER_VERSION}' PROMTAIL_VERSION='${PROMTAIL_VERSION}' LOKI_VERSION='${LOKI_VERSION}' PROMETHEUS_VERSION='${PROMETHEUS_VERSION}' GRAFANA_VERSION='${GRAFANA_VERSION}' GRAFANA_RPM_RELEASE='${GRAFANA_RPM_RELEASE}' bash -s" <<'PROVISION_BINS'
set -euo pipefail
...
# ==========================================================================
echo ""
echo "=== Phase 5: Packaging golden box ==="
...
echo ""
echo "  Next steps:"
echo "    1. vagrant box add --name netwatch-golden artifacts/boxes/netwatch-golden.box --force"
